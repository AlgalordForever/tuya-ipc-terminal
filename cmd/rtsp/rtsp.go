...
func runStartServer(cmd *cobra.Command, args []string) error {
    ...
    // 1. Configuração do Contexto do Daemon
    cntxt := &daemon.Context{
        PidFileName: "tuya-rtsp.pid",
        PidFilePerm: 0644,
        LogFileName: "tuya-rtsp.log",
        LogFilePerm: 0640,
        WorkDir:     "./",
        Umask:       027,
    }
    ...
    if isDaemon {
        d, err := cntxt.Reborn()
        if err != nil {
            return fmt.Errorf("unable to run as daemon: %v", err)
        }
        if d != nil {
            // Este é o processo pai. Ele avisa que o filho nasceu e sai.
            core.Logger.Info().Msgf("RTSP server starting in background (PID: %d)", d.Pid)
            return nil
        }
        // Se d == nil, estamos no processo FILHO.
        defer cntxt.Release()
    }

    // 2. Lógica de Inicialização (Agora roda no processo correto)
    
    // IMPORTANTE: Se for daemon, precisamos re-inicializar o Logger ou Storage
    // se eles dependerem de estados do processo pai que não foram herdados.
    
    users, err := storageManager.ListUsers()
    if err != nil || len(users) == 0 {
        return errors.New("no authenticated users found")
    }
    ...
    if isDaemon {
        // No modo daemon, apenas esperamos os sinais de parada
        <-signalChan
    } else {
        core.Logger.Info().Msgf("RTSP server is running. Press Ctrl+C to stop.")
        <-signalChan
...
*/
func runServerStatus(cmd *cobra.Command, args []string) error {
    // 1. Verificar se o arquivo PID existe
    pidData, err := os.ReadFile("tuya-rtsp.pid")
    if err != nil {
        if os.IsNotExist(err) {
            fmt.Println("RTSP Server Status: Stopped (No PID file found)")
        } else {
    ...
    // 2. Tentar converter o conteúdo do arquivo para um número de processo
    var pid int
    _, err = fmt.Sscanf(string(pidData), "%d", &pid)
    if err != nil {
        fmt.Println("RTSP Server Status: Error (Invalid PID file)")
        return nil
    }

    // 3. Verificar se o processo realmente existe no sistema operacional
    process, err := os.FindProcess(pid)
    if err != nil {
        fmt.Printf("RTSP Server Status: Stopped (PID %d not found)\n", pid)
        return nil
    }

    // No Unix, FindProcess sempre retorna algo, então usamos o sinal 0 para checar vida
    err = process.Signal(syscall.Signal(0))
    if err != nil {
        fmt.Printf("RTSP Server Status: Stopped (Process %d is not responding)\n", pid)
        // Opcional: remover o arquivo PID órfão aqui
        return nil
    }

    // 4. Se chegamos aqui, o Daemon está rodando
    fmt.Println("RTSP Server Status:")
    fmt.Println("==================")
    fmt.Printf("Status:   RUNNING (as Daemon)\n")
    fmt.Printf("PID:      %d\n", pid)
    fmt.Println("Note: Detailed stats (client count, etc.) require IPC or a shared state file.")
