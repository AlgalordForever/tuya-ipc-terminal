...
func (s *RTSPServer) Start() error {
    ...
    // Tenta abrir o listener antes de mudar o estado
    ...
    // Listar endpoints (opcional, mas bom para log)
    if err := s.printAvailableEndpoints(); err != nil {
        core.Logger.Warn().Msgf("Could not list camera endpoints: %v", err)
    }

    // Iniciamos as rotinas em background
    go s.acceptConnections()
    go s.cleanupRoutine()

    // Pequena pausa para garantir que as goroutines escalonem 
    // e o estado seja propagado antes do retorno da função
...
*/
func (s *RTSPServer) acceptConnections() {
    // IMPORTANTE: Não usamos select com default aqui.
    // O Accept() é bloqueante e deve ser interrompido pelo fechamento do listener.
    for {
        conn, err := s.listener.Accept()
        if err != nil {
            s.mutex.RLock()
            isStillRunning := s.running
            s.mutex.RUnlock()

            if !isStillRunning {
                // Servidor parou intencionalmente
                return
            }
